...
Promise.all(['candidates','governments','local_cultural_issues','civic_policy_calls','region_metrics'].map(x=>fetch('data/'+x+'.json').then(r=>r.json()))).then(([p,g,i,c,m])=>{
  ...
  regions.innerHTML=cities.map(city=>{
    ...
    return `<article class="card region-card"><div class="card-meta"><span class="tag">${city}</span><span class="tag">${stats?.year||'待查核'} 文化統計</span></div><h3>${city}文化 Board</h3><p class="summary">藝文活動 ${stats?.arts_events_total?.toLocaleString('zh-TW')||'—'} 個 · 文化展演場地 ${stats?.cultural_venues_total?.toLocaleString('zh-TW')||'—'} 處<br>候選人 ${new Set(n.map(x=>x.candidate)).size} 位 · 政見 ${n.length} 筆 · 地區議題 ${q.length} 筆 · 預算資料 ${b.length} 筆</p><div class="source-row"><a href="region.html?city=${encodeURIComponent(city)}">查看 Board →</a></div></article>`;
  }).join('');
