...
Promise.all(['candidates','governments','local_cultural_issues','region_metrics'].map(x=>fetch('data/'+x+'.json').then(r=>r.json()))).then(([p,g,i,m])=>{
  ...
  budget.innerHTML=G.length?G.map(x=>`<article class="gov-card"><h3>${escapeHtml(x.year)} ${escapeHtml(x.city)}</h3><div class="metric"><span>文化局／文化處預算</span><strong>${escapeHtml(x.culture_budget_display)}</strong></div><div class="metric"><span>政事別文化支出</span><strong>${escapeHtml(x.actual_spending_display)}</strong></div><p>${escapeHtml(x.methodology)}</p><a href="${escapeHtml(safeUrl(x.official_source_url))}" target="_blank" rel="noopener">官方來源 ↗</a></article>`).join(''):'<p class="empty empty-dark">預算與支出資料尚待查核。</p>';
  candidates.innerHTML=P.map(x=>`<article class="card"><h3>${escapeHtml(x.candidate)}</h3><span class="party">${escapeHtml(x.party)}</span><p class="summary">${escapeHtml(x.summary)}</p></article>`).join('')||'<p class="empty">候選人政見尚待收錄。</p>';
