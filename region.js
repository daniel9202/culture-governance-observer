...
Promise.all([loadCandidateDataset(),...['governments','local_cultural_issues','region_metrics'].map(x=>fetch('data/'+x+'.json').then(r=>r.json()))]).then(([p,g,i,m])=>{
  const rawPolicies=p.records.filter(x=>x.city===city),P=groupCandidateRecords(rawPolicies,(p.shared_policy_groups||[]).filter(x=>x.city===city)),G=g.records.filter(x=>x.city===city),I=i.records.filter(x=>x.city===city),M=m.records.find(x=>x.city===city),topics=[...new Set(P.flatMap(x=>x.topics))];
  ...
  const sources=x=>x.sources.length?`<ul class="source-list">${x.sources.map((source,index)=>`<li><a class="source-link" href="${escapeHtml(safeUrl(source.url))}" target="_blank" rel="noopener">${sourceContent(source,index)}<span aria-hidden="true">↗</span></a></li>`).join('')}</ul>`:'<p>尚未收錄來源。</p>';
  candidates.innerHTML=P.map(x=>`<article class="card candidate-card"><div class="card-meta"><span class="tag">${escapeHtml(x.office)}</span>${x.topics.map(t=>`<span class="tag">${escapeHtml(t)}</span>`).join('')}</div><h3>${escapeHtml(x.candidate)}</h3><span class="party">${escapeHtml(x.party)}</span><div class="policy-layer"><h4>政策論述</h4>${list(x.policy_arguments,'尚未收錄可核實的政策論述。')}</div><div class="policy-layer policy-actions"><h4>具體主張</h4>${list(x.concrete_proposals,'尚未收錄具體主張。')}</div><div class="policy-layer policy-statements"><h4>相關發言</h4>${list(x.related_statements,'尚未收錄可核實的相關發言。')}</div>${String(x.office).includes('議員')?`${sharedPolicySection('政黨共同政見',x.party_shared_policies||[])}${sharedPolicySection('區域共同政見',x.regional_shared_policies||[])}`:''}<div class="policy-layer policy-sources"><h4>相關來源</h4>${sources(x)}</div><small class="verification">發布：${escapeHtml(x.published_dates.join('、')||'待查核')} · 最後查核：${escapeHtml(x.last_verified)} · 更正 ${Number(x.corrections.length)||0} 次</small></article>`).join('')||'<p class="empty">候選人政見尚待收錄。</p>';
