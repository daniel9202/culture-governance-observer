...
Promise.all([loadCandidateDataset(),...['governments','local_cultural_issues','region_metrics'].map(x=>fetch('data/'+x+'.json').then(r=>r.json()))]).then(([p,g,i,m])=>{
  ...
  const sharedPolicyList=record=>record.shared_policies.length?record.shared_policies.map(policy=>`<section class="shared-policy-item"><h5>${escapeHtml(policy.title)}</h5><p>${escapeHtml(policy.summary)}</p>${list(policy.concrete_proposals||[],'尚未收錄具體主張。')}<a href="${escapeHtml(safeUrl(policy.source_url))}" target="_blank" rel="noopener">${escapeHtml(policy.source_title||'查看共同政見來源')} ↗</a></section>`).join(''):'<p>尚未收錄可核實的共同政見。</p>';
  ...
  candidates.innerHTML=P.map(x=>`<article class="card candidate-card"><div class="card-meta"><span class="tag">${escapeHtml(x.office)}</span>${x.topics.map(t=>`<span class="tag">${escapeHtml(t)}</span>`).join('')}</div><h3>${escapeHtml(x.candidate)}</h3><span class="party">${escapeHtml(x.party)}</span><div class="policy-layer"><h4>政策論述</h4>${list(x.policy_arguments,'尚未收錄可核實的政策論述。')}</div><div class="policy-layer policy-actions"><h4>具體主張</h4>${list(x.concrete_proposals,'尚未收錄具體主張。')}</div><div class="policy-layer policy-statements"><h4>相關發言</h4>${list(x.related_statements,'尚未收錄可核實的相關發言。')}</div><div class="policy-layer policy-shared"><h4>共同政見</h4>${sharedPolicyList(x)}</div><div class="policy-layer policy-sources"><h4>相關來源</h4>${sources(x)}</div><small class="verification">發布：${escapeHtml(x.published_dates.join('、')||'待查核')} · 最後查核：${escapeHtml(x.last_verified)} · 更正 ${Number(x.corrections.length)||0} 次</small></article>`).join('')||'<p class="empty">候選人政見尚待收錄。</p>';
