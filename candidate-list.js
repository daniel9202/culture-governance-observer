...
function render(){
  ...
  cards.innerHTML=rows.map(x=>`<article class="card candidate-card"><div class="card-meta"><span class="tag">${escapeHtml(x.city)}</span><span class="tag">${escapeHtml(x.office)}</span>${x.topics.map(t=>`<span class="tag">${escapeHtml(t)}</span>`).join('')}</div><h3>${escapeHtml(x.candidate)}</h3><span class="party">${escapeHtml(x.party)}</span><div class="policy-layer"><h4>政策論述</h4>${bulletList(x.policy_arguments,'尚未收錄可核實的政策論述。')}</div><div class="policy-layer policy-actions"><h4>具體主張</h4>${bulletList(x.concrete_proposals,'尚未收錄具體主張。')}</div><div class="policy-layer policy-statements"><h4>相關發言</h4>${bulletList(x.related_statements,'尚未收錄可核實的相關發言。')}</div>${shouldShowSharedPolicies?`<div class="policy-layer policy-shared"><h4>共同政見</h4>${sharedPolicyList(x)}</div>`:``}<div class="policy-layer policy-sources"><h4>相關來源</h4>${sourceList(x)}</div><small class="verification">發布：${escapeHtml(x.published_dates.join('、')||'待查核')} · 最後查核：${escapeHtml(x.last_verified)} · 更正 ${Number(x.corrections.length)||0} 次</small></article>`).join('');
