...
def update(p):
 ...
 for x in ("review_status","review_note"):
  if x not in fields: fields.append(x)
 found=[r for r in rows if r.get("source_url")==url]
 if not found: raise ValueError("找不到這筆來源；請重新整理後再試")
 for r in found:r["review_status"],r["review_note"]=status,note
 with path.open("w",encoding="utf-8",newline="") as f:
  w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(rows)
 rel=str(path.relative_to(ROOT));git("add","--",rel)
 if subprocess.run(["git","diff","--cached","--quiet"],cwd=ROOT).returncode==0:return "沒有資料變動"
 git("commit","-m",f"data: review {kind} source");git("push");return git("rev-parse","--short","HEAD")
class H(SimpleHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  except (ValueError,json.JSONDecodeError) as e:self.reply(400,{"error":str(e)})
  except Exception as e:self.reply(500,{"error":str(e)})
 def reply(self,code,data):
  b=json.dumps(data,ensure_ascii=False).encode();self.send_response(code);self.send_header("Content-Type","application/json; charset=utf-8");self.send_header("Content-Length",str(len(b)));self.end_headers();self.wfile.write(b)
