...
<main><a href="https://daniel9202.github.io/culture-governance-observer/">← 回公開網站</a><p>REVIEW INBOX</p><h1>待審核收件匣</h1><p><strong>審核收件匣</strong>　<a href="/review/analytics">流量統計</a></p><p>候選人政見與民間文化訴求均在此核對、修正後上架。</p><details><summary>新增待審草稿</summary><form id="new-candidate"><label>原始來源網址<input name="source_url" type="url" required></label><label>來源標題<input name="source_title" required></label><label>發布日期<input name="published_date" type="date"></label><label>候選人<input name="candidate" required></label><label>政黨<input name="party"></label><label>職務<select name="office"><option>縣市長</option><option>縣市議員</option></select></label><label>縣市<input name="city" required></label><label>文化議題（以 | 分隔）<input name="topics"></label><label>摘要<textarea name="summary" required></textarea></label><button>建立待審草稿</button></form></details><label>資料類型<select id="kind-filter"><option value="">全部待審資料</option><option value="candidate">候選人政見</option><option value="civic_call">民間訴求</option></select></label><p id="status">正在讀取審核資料…</p><button id="reload">重新整理</button> <strong id="count"></strong><div id="rows"></div><details><summary>已處理 <span id="handled-count">0 筆</span></summary><div id="handled"></div></details></main>
<script>
...
async function createCandidate(event){event.preventDefault();let form=event.currentTarget,fields=Object.fromEntries(new FormData(form)),payload={...fields,topics:fields.topics.split('|').map(x=>x.trim()).filter(Boolean)},result=await f('/api/review/items',{method:'POST',body:JSON.stringify({kind:'candidate',...fields,payload})});if(!result.ok){let detail=await result.json().catch(()=>({}));s.textContent=detail.error||'建立草稿失敗';return}form.reset();s.textContent='已建立待審草稿。';load()}
document.querySelector('#new-candidate').onsubmit=createCandidate;document.querySelector('#reload').onclick=()=>load().catch(x=>s.textContent=x.message);kindFilter.onchange=()=>load().catch(x=>s.textContent=x.message);load().catch(x=>s.textContent=x.message);
</script></html>`;
}
